...
def parse_recursive(tag: bs4.element.PageElement, decorator) -> list[Fragment]:
    ...
    if isinstance(tag, bs4.element.Comment):
        ...
        if len(fragments) > 0:
            if fragments[-1].content == "Click here  for the corresponding HTML documentation that is generated by doxygen.":
                fragments = fragments[:-1]
            elif fragments[-1].content == "Go to the next section or return to the index.":
                fragments = fragments[:-1]
        ...
        # The "Click here" part is useless in the Visual Studio autocomplete box, since one cannot click there.
        filtered_fragments = [f for f in fragments if f.content.strip() != "Click here\n  for the corresponding HTML documentation that is generated by doxygen."]
        if len(fragments) != len(filtered_fragments):
            # Strip some trailing whitespace that is now obsolete due to the removed "Click here" string.
            filtered_fragments = rstrip_fragments(filtered_fragments)
